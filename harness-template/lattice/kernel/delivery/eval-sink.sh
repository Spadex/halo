...
EVAL_DIR="lattice/state/eval-runs"
OUTCOME_DIR="lattice/state/outcomes"
...
write_project_manifest() {
  ...
  {
    printf '{\n'
    printf '  "schema_version": "lattice.eval-sink-project.v1",\n'
...
render_index() {
  ...
  echo "# Lattice Central Eval Sink"
  echo ""
...
case "$ACTION" in
  publish)
    ...
    REPORT_COPIED=0
    if [[ -d "$PROJECT_ROOT/lattice/state" ]]; then
      REPORT_COPIED="$(copy_matching_files "$PROJECT_ROOT/lattice/state" "$PROJECT_SINK/reports" '*.md')"
    fi
