...
- 不把大段文档复制进 `spec.md`；只把影响 scope、AC、risk、interface、compatibility 或 verification 的事实写入 `halo/specs/<spec-id>/spec.md` 的 Context Basis。

## 冲突优先级

1. 用户本次明确指令。
2. 当前代码、测试、schema 和接口契约。
3. `halo/context/knowledge/` 下的项目知识。
4. `halo/specs/` 下的历史 spec。
5. `external.md` 和中心知识。
6. 模型先验。

有实质影响的冲突必须记录到 `halo/specs/<spec-id>/spec.md` 的 Context Basis。
