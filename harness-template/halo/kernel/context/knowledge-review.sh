...
target_path() {
  ...
  case "$abs" in
    "$PROJECT_ROOT/halo/context/drafts/"*|"$PROJECT_ROOT/halo/context/knowledge/"*) printf '%s' "$abs" ;;
    *) echo "Target must be under halo/context/drafts/ or halo/context/knowledge/: $(rel_path "$abs")"; exit 1 ;;
  esac
}
...
OUT="$PROJECT_ROOT/halo/state/knowledge-reviews/${EVENT_ID}.json"
mkdir -p "$(dirname "$OUT")"

{
  printf '{\n'
  printf '  "schema_version": "halo.knowledge-review.v1",\n'
