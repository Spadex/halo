...
resolve_source_file() {
  ...
  if [[ "$input" == *.md || "$input" == */* ]]; then
    ...
    abs="$PROJECT_ROOT/halo/specs/$input/verify.md"
  fi
...
if [[ -z "$OUT" ]]; then
  OUT="$PROJECT_ROOT/halo/context/drafts/knowledge-${SPEC_ID}-$(date -u +%Y%m%dT%H%M%SZ).md"
elif [[ "$OUT" != /* ]]; then
  OUT="$PROJECT_ROOT/$OUT"
fi

case "$OUT" in
  "$PROJECT_ROOT/halo/context/drafts/"*) ;;
  *) echo "Output must be under halo/context/drafts/: $(rel_path "$OUT")"; exit 1 ;;
esac
