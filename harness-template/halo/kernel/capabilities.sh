#!/usr/bin/env bash
# capabilities.sh — Halo runtime capability declaration.
# This is the stable discovery surface for IDE adapters and MCP servers.
source "$(dirname "$0")/_lib.sh"

for arg in "$@"; do
  [[ "$arg" == "--help" || "$arg" == "-h" ]] && cli_help "capabilities" "Print Halo runtime capabilities as JSON" \
...
[
    {
      "id": "capabilities",
      "label": "Capabilities",
      "command": "bash halo/kernel/capabilities.sh --json",
      "available": $(has_executable "halo/kernel/capabilities.sh")
    },
    {
      "id": "guide",
      "label": "Guide",
    ...
    },
    {
      "id": "doctor",
      "label": "Doctor",
      "command": "bash halo/kernel/doctor.sh",
      "available": $(has_executable "halo/kernel/doctor.sh")
    },
    {
      "id": "run_pipeline",
      "label": "Run Pipeline",
      "command": "bash halo/kernel/delivery/pipeline.sh --json-out",
      "available": $(has_executable "halo/kernel/delivery/pipeline.sh")
    },
    {
      "id": "eval_query",
      "label": "Eval Query",
      "command": "bash halo/kernel/delivery/eval-query.sh summary --format=json",
      "available": $(has_executable "halo/kernel/delivery/eval-query.sh")
    },
    {
      "id": "record_outcome",
      "label": "Record Outcome",
      "command": "bash halo/kernel/delivery/outcome-link.sh record",
      "available": $(has_executable "halo/kernel/delivery/outcome-link.sh")
    },
    {
      "id": "open_dashboard",
      "label": "Open Dashboard",
      "command": "bash halo/kernel/delivery/eval-dashboard.sh",
      "available": $(has_executable "halo/kernel/delivery/eval-dashboard.sh")
    }
  ]
JSON
}
...
[
    {
      ...
      "kind": "primary",
      "tool": "guide",
      "description": "Resolve the next Halo action from current repo artifacts.",
...
[
    {
      ...
      "label": "Eval Summary",
      "kind": "markdown",
      "command": "bash halo/kernel/delivery/eval-summary.sh <eval-json>"
    },
    {
      "id": "eval_history",
      "label": "Eval History",
      "kind": "markdown",
      "command": "bash halo/kernel/delivery/eval-history.sh"
    },
    {
      "id": "eval_dashboard",
...
cat <<JSON
{
  "schema_version": "halo.capabilities.v1",
  "kind": "halo-capabilities",
  "runtime": {
    "name": "halo",
  ...
  "host": {
    "mode": "halo",
    "manifest": "halo/manifest.yaml",
    "specs_dir": "$(json_escape "${specs_dir:-halo/specs}")",
    "eval_sink_dir": "$(json_escape "${eval_sink_dir:-halo/state/eval-sink}")"
  },
  "policy": {
