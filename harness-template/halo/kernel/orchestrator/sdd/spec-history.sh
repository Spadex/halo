...
for arg in "$@"; do
  [[ "$arg" == "--help" || "$arg" == "-h" ]] && cli_help "spec history" "Aggregate spec transition JSON events into a Markdown report" \
    "spec-history.sh                                      Print history from halo/state/spec-transitions" \
    "spec-history.sh --out=halo/state/spec-history.md  Write Markdown report" \
    "spec-history.sh --dir=<dir> --limit=20               Use custom event directory"
done

EVENT_DIR="$PROJECT_ROOT/halo/state/spec-transitions"
OUT=""
LIMIT=20
