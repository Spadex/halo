...
if [[ -z "$OUT" ]]; then
  ...
  if [[ "$TASK_ID" == "branch" ]]; then
    MD_OUT="$PROJECT_ROOT/halo/specs/$SPEC_ID/review.md"
  else
...
REVIEW_PACKAGE=".halo/sdd/$SPEC_ID/$TASK_ID/review-package.md"
scope_label() {
  if [[ "$TASK_ID" == "branch" ]]; then
    printf '整体分支'
  else
...
{
  printf '{\n'
  printf '  "schema_version": "halo.review-summary.v1",\n'
...
{
  printf -- '---\n'
  printf 'schema_version: halo.review.v1\n'
  printf 'kind: review\n'
  printf 'spec_id: "%s"\n' "$SPEC_ID"
  printf 'task_id: "%s"\n' "$TASK_ID"
  printf 'created_at: "%s"\n' "$CREATED_AT"
  ...
  printf '## 3. 检查范围\n\n'
  printf -- '- 技术方案：`halo/specs/%s/spec.md`\n' "$SPEC_ID"
  printf -- '- 实施计划：`halo/specs/%s/plan.md`\n' "$SPEC_ID"
