...
for arg in "$@"; do
  [[ "$arg" == "--help" || "$arg" == "-h" ]] && cli_help "outcome link" "Record outcome evidence linked to a Halo eval run" \
...
resolve_eval_json() {
  ...
  if [[ "$input" == /* ]]; then
    ...
    path="$PROJECT_ROOT/halo/state/eval-runs/$input.json"
  fi
...
if [[ -z "$OUT" ]]; then
  OUT="$PROJECT_ROOT/halo/state/outcomes/${OUTCOME_ID}.json"
elif [[ "$OUT" != /* ]]; then
...
{
  printf '{\n'
  printf '  "schema_version": "halo.outcome-link.v1",\n'
