...
RUN_ID="${RUN_ID//-/}"
RUN_ID="${RUN_ID}-$$"
GATE_JSON_DIR="$PROJECT_ROOT/halo/state/eval-runs/${RUN_ID}.gates"
LOOP_JSON_FILE=""
LEARN_DRAFT_FILE=""
gate_json_files=()
...
collect_process_evidence() {
  ...
  [[ -n "$spec_id" ]] || return 0
  evidence_dir="$PROJECT_ROOT/.halo/sdd/$spec_id"
...
resolve_failure_categories_file() {
  ...
  [[ -n "$configured" ]] || configured="halo/config/failure-categories.yaml"
  if [[ "$configured" == /* ]]; then
...
resolve_eval_json_out() {
  ...
  if [[ -z "$JSON_OUT" ]]; then
    EVAL_JSON_FILE="$PROJECT_ROOT/halo/state/eval-runs/${RUN_ID}.json"
  elif [[ "$JSON_OUT" == /* ]]; then
...
write_learn_draft() {
  ...
  out="$PROJECT_ROOT/halo/context/drafts/escalation-${RUN_ID}.md"
  loop_out="$PROJECT_ROOT/halo/state/loops/${RUN_ID}.json"
  mkdir -p "$(dirname "$out")"
...

  {
    ...
    printf 'schema_version: halo.learn-draft.v1\n'
    printf 'status: draft\n'
    ...
    printf '%s\n' '- Applies when:'
    printf '%s\n' '- Guidance:'
    printf '%s\n\n' '- Target knowledge file: `halo/context/knowledge/`'
    ...
    printf '%s\n' '- [ ] Promote to `halo/context/knowledge/` or discard this draft.'
  } > "$out"
...
write_loop_json() {
  ...
  out="$PROJECT_ROOT/halo/state/loops/${RUN_ID}.json"
  mkdir -p "$(dirname "$out")"
  ...
  {
    printf '{\n'
    printf '  "schema_version": "halo.loop-state.v1",\n'
...
write_eval_json() {
  ...
  spec_rel="${SPEC_FILE#$PROJECT_ROOT/}"
  agent_name="${HALO_AGENT:-${AGENT_NAME:-unknown}}"
