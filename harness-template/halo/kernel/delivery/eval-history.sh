...
for arg in "$@"; do
  [[ "$arg" == "--help" || "$arg" == "-h" ]] && cli_help "eval history" "Aggregate eval run JSON files into a Markdown report" \
    "eval-history.sh                                      Print history report from halo/state/eval-runs" \
...
EVAL_DIR="$PROJECT_ROOT/halo/state/eval-runs"
OUTCOME_DIR="$PROJECT_ROOT/halo/state/outcomes"
OUT=""
LIMIT=20
...
render_history() {
  ...
  echo "| Run | Status | Spec | Git | AC | Drift | Review | TDD | Loop | Outcomes |"
  echo "|---|---|---|---|---|---|---|---|---|---|"
  ...
  local i file run_id status spec git ac_total ac_covered drift review_total review_failed review_cannot_verify tdd_total tdd_invalid retry_count next_action failure_category learn_draft learn_flag outcome_summary
  for ((i = RUN_TOTAL - 1; i >= start_index; i--)); do
    ...
    echo "| $(md_escape "${run_id:-unknown}") | $(md_escape "${status:-unknown}") | $(md_escape "${spec:-none}") | $(md_escape "${git:-unknown}") | $ac_covered/$ac_total | $drift | $review_failed fail / $review_cannot_verify cannot_verify / $review_total | $tdd_invalid invalid / $tdd_total | retry=$retry_count, next=$(md_escape "${next_action:-unknown}"), category=$(md_escape "${failure_category:-none}"), learn=$learn_flag | $(md_escape "$outcome_summary") |"
  done
}
