...
for arg in "$@"; do
  [[ "$arg" == "--help" || "$arg" == "-h" ]] && cli_help "outcome report" "Summarize outcome links and attribution signals" \
    "outcome-report.sh                                      Print Markdown report from halo/state/outcomes" \
...
TMP_REFS="$(mktemp "${TMPDIR:-/tmp}/halo-outcome-refs.XXXXXX")"
cleanup() {
  rm -f "$TMP_REFS"
}
trap cleanup EXIT
...
render_report() {
  echo "# Halo Outcome Attribution Report"
  echo ""
