...
if [[ -z "$CONFIG_FILE" ]]; then
  ...
  [[ -n "$CONFIG_FILE" ]] || CONFIG_FILE="halo/config/failure-categories.yaml"
fi
...
if [[ "$SCHEMA_VERSION" == "halo.failure-categories.v1" ]]; then
  pass "schema_version"
else
  fail "schema_version must be halo.failure-categories.v1"
fi
