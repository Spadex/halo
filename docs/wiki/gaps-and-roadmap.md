...
| Evidence 已有 pipeline/gate JSON、loop state、可配置 failure category、escalation learn draft、promotion event、knowledge governance lint、process evidence、Markdown summary、history report、CI artifact 与 PR comment 模板，但 context 采用记录仍弱 | 不知道哪些 context 真的提升了交付质量 | context-runs + metadata |
...
- context metadata 和 context-runs。
...
2. 给 context/knowledge 增加 metadata 和 context-runs。
