...
| Evidence 已有 pipeline/gate JSON、process evidence、Markdown summary、history report、CI artifact 与 PR comment 模板，但 loop 状态仍弱 | 修复轮次和升级原因难审计 | 推进 loop state JSON |

### P1：Evidence 与 Loop

| Gap | 影响 | 建议 |
|-----|------|------|
| Retry/loop 状态不落盘 | 修复轮次不可审计 | loop state JSON |
