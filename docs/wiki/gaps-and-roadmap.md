...
| 方向 | 当前证据 | 判断 |
|------|----------|------|
| PrismSpec | canonical skills、guide.sh、lint.sh、spec-state-lint、spec-status、plan-lint、task-evidence-lint、多模板、Plan/TDD policy | 主链路成立 |
...
| Evidence 已有 pipeline/gate JSON、spec-state-lint、spec-status、spec transition events、plan-lint、task-evidence-lint、context-lint、context-run、loop state、outcome link/report、central eval sink、static dashboard、eval query、可配置 failure category、escalation learn draft、knowledge review event、promotion event、knowledge metadata/governance lint、process evidence、Markdown summary、history report、CI artifact 与 PR comment 模板 | 数据已经可汇总和查询，团队视角仍需要更强趋势分析 | dashboard trend |
