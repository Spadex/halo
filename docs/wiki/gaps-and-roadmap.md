...
| Evidence 已有 pipeline/gate JSON、loop state、可配置 failure category、escalation learn draft、promotion event、process evidence、Markdown summary、history report、CI artifact 与 PR comment 模板，但 knowledge 治理仍弱 | 候选经验可能重复、过期或冲突 | knowledge metadata + conflict/stale diagnostics |

### P1：Evidence 与 Loop

| Gap | 影响 | 建议 |
|-----|------|------|
| learn promotion 质量治理不足 | 候选经验可晋升/废弃，但晋升前缺少重复、冲突、过期提示 | duplicate/conflict/stale diagnostics |
...
| promotion 前质量判断仍靠人工 | 经验候选晋升前缺少结构化 metadata 与冲突提示 | metadata + conflict/stale diagnostics |
...
- promotion duplicate/conflict/stale diagnostics。
...
2. 增加 promotion duplicate/conflict/stale diagnostics。
