...
| Evidence 已有 pipeline/gate JSON、context-run、loop state、outcome link/report、central eval sink、static dashboard、可配置 failure category、escalation learn draft、knowledge review event、promotion event、knowledge metadata/governance lint、process evidence、Markdown summary、history report、CI artifact 与 PR comment 模板 | 数据已经可汇总，团队视角仍需要更强查询和趋势分析 | dashboard query |
...
| Dashboard 仍是静态文件 | central sink 已有文件协议和本地 HTML，但缺少交互过滤、跨项目趋势和查询 API | dashboard query |
...
| context-run 仍偏计数型 | 已能关联 outcome event 并生成线索报告，但真实样本和跨项目归因不足 | cross-project attribution |
...
- dashboard query/trend improvements。
