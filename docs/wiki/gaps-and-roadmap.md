...
| Gap | 影响 | 建议 |
|-----|------|------|
| Clarify 与 Spec 仍共用 specification skill | 用户看到五板块，但底层还没有独立 clarify 状态 | 先增强 `context.md` 的 blocking questions / assumptions / conflicts 区块和 lint |
| Spec review 仍偏弱 | AC 可测性、scope 过大、risk 缺失仍依赖 Agent 自觉 | 增加 spec-review evidence 或 spec semantic checklist gate |
| Review runtime 不完整 | 已有 reviewer persona 和 summary JSON，但 dispatch/finding lifecycle 仍靠 Agent | 建 review package -> reviewer -> summary -> blocking gate 的标准执行路径 |
...
- 选择 Lumi 作为真实业务示例。
- 从需求生成 `context.md`、`spec.md`、`plan.md`、review evidence 和 `verify.md`。
- 记录遇到的 Lattice gap。
