...
| Evidence 已有 pipeline/gate JSON，但语义证据仍弱 | review/TDD/人工检查难进入趋势 | 推进 review JSON、TDD evidence、CI artifact |
...
| CI artifact 约定较弱 | eval run 难进入 PR 和趋势分析 | GitHub Actions 上传 eval JSON |
...
2. 做 CI artifact contract，让 PR 能上传和展示 eval run。
