...
| Evidence 已有 pipeline JSON，但语义证据仍弱 | review/TDD/人工检查难进入趋势 | 推进 gate JSON、review JSON、TDD evidence |

### P1：Evidence 与 Loop

| Gap | 影响 | 建议 |
|-----|------|------|
| gate 结果未结构化 | AC/drift/compliance 还难做趋势分析 | gate JSON output contract |
...
2. 做 gate output JSON contract，把 AC coverage / drift findings 接入 eval run。
3. 做 review/TDD evidence JSON。
4. 给 context/knowledge 增加 metadata 和 context-runs。
5. 扩展 Node/Python drift parser 和插件协议。
