...
| Evidence 已有 pipeline/gate JSON、context-run、loop state、可配置 failure category、escalation learn draft、knowledge review event、promotion event、knowledge metadata/governance lint、process evidence、Markdown summary、history report、CI artifact 与 PR comment 模板，但 context outcome linkage 仍弱 | 不知道哪些 context 真的降低返工或缺陷 | outcome linkage |
