...
| Evidence 已有 pipeline/gate JSON、context-run、loop state、可配置 failure category、escalation learn draft、promotion event、knowledge governance lint、process evidence、Markdown summary、history report、CI artifact 与 PR comment 模板，但 context outcome linkage 仍弱 | 不知道哪些 context 真的降低返工或缺陷 | outcome linkage + metadata |

### P1：Evidence 与 Loop

| Gap | 影响 | 建议 |
|-----|------|------|
| learn promotion 质量治理仍偏轻量 | 已有 source/placeholder/conflict/expiry lint，但缺少语义级冲突和 owner 治理 | metadata + reviewer policy |
...
- context metadata 和 outcome linkage。
...
2. 给 context/knowledge 增加 metadata 和 outcome linkage。
