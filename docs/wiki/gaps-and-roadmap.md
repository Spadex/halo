...
| Context | context map、project knowledge、external entry、knowledge backend、knowledge governance lint、knowledge review evidence、sync | 最小可用 |
| Delivery | pipeline、spec-lint、PrismSpec lint、AC coverage、drift、compliance、spec-lock、outcome link | 最小验证闭环成立 |
...
| Evidence 已有 pipeline/gate JSON、context-run、loop state、outcome link、可配置 failure category、escalation learn draft、knowledge review event、promotion event、knowledge metadata/governance lint、process evidence、Markdown summary、history report、CI artifact 与 PR comment 模板，但 outcome attribution 仍弱 | 知道发生了什么，但还不能自动判断哪些 context 真的降低返工或缺陷 | dashboard / attribution analysis |
...
| context-run 仍偏计数型 | 已能关联 outcome event，但真实样本和归因分析不足 | outcome attribution |
...
- outcome event 能关联 eval run，并进入 summary/history。
...
- context-run 与 outcome event 的归因分析。
