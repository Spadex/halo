...
| 方向 | 当前证据 | 判断 |
|------|----------|------|
| SDD | Spec 模板、spec-lint、AC 编号、Brainstorming/Planning/Implementation/Finishing skills、plan/tdd 执行策略、task brief、review package | 路线成立，下一步是把状态、计划与 review verdict 机器化 |
...
| Gap | 影响 | 建议 |
|-----|------|------|
| SDD skills 仍以 Markdown 指令为主 | 阶段输入/输出还不能被完全机器强制 | 增加 CLI/gate 或 structured evidence |
| Spec 状态未被 gate 消费 | 不知道 drafted/planned/implemented/verified/finished 是否可信 | 让 `spec-lint` 校验 front matter |
| Execution policy 未被 gate 消费 | Plan Mode 和 TDD Mode 的约束无法被强制区分 | 校验 `execution_mode: plan | tdd` 并联动 AC coverage |
| Plan 未被 gate 校验 | plan 与 AC 可能脱节 | 新增 `plan-lint.sh` |
| TDD/review evidence 仍偏文本 | 无法自动确认 red-before-green、AC-to-test、review verdict | 增加 TDD/review evidence JSON 或 gate |
...
- 新增 `plan-lint.sh`。
- Smoke test 覆盖 skills 安装、modern spec lint、pipeline/eval JSON。
...
4. 增加 plan-lint 和 knowledge-lint。
