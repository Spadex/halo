...
| 方向 | 当前证据 | 判断 |
|------|----------|------|
| SDD | Spec 模板、spec-lint、AC 编号、Brainstorming/Planning/Implementation/Finishing skills、plan/tdd 执行策略 | 路线成立，下一步是把状态、计划与 TDD 证据机器化 |
...
| Gap | 影响 | 建议 |
|-----|------|------|
| SDD skills 仍是 Markdown 指令 | 阶段输入/输出还不能被机器强制 | 增加 CLI/gate 或 structured evidence |
...
| TDD evidence 不可机读 | 无法确认 red-before-green 和 AC-to-test 关系 | 增加 TDD evidence 记录或 gate |
