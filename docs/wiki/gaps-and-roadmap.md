...
| Context | context map、project knowledge、external entry、knowledge backend、knowledge governance lint、sync | 最小可用 |
...
| Evidence 已有 pipeline/gate JSON、context-run、loop state、可配置 failure category、escalation learn draft、promotion event、knowledge metadata/governance lint、process evidence、Markdown summary、history report、CI artifact 与 PR comment 模板，但 context outcome linkage 仍弱 | 不知道哪些 context 真的降低返工或缺陷 | outcome linkage |

### P1：Evidence 与 Loop

| Gap | 影响 | 建议 |
|-----|------|------|
| learn promotion 质量治理仍偏轻量 | 已有 metadata/source/placeholder/conflict/expiry lint，但缺少语义级冲突和 reviewer policy | reviewer policy |
...
| central sync 仍是轻量文件同步 | 只能解决基础共享，不能解决治理 | 增加 reviewer、source trust、staleness policy |
| promotion 前质量判断仍需要 reviewer | lint 能发现结构风险，但不能判断语义正确性 | reviewer policy + conflict resolution |
| context-run 仍偏计数型 | 知道采用了什么，但不知道是否真的提升交付质量 | 关联 review finding、返工和 defect |
...
- outcome linkage 和 reviewer policy。
...
- learn draft 可以被 promote 或 discard，并留下审计事件。
...
- 增加 reviewer policy 和跨文件 conflict resolution。
- context-run 关联 review finding、返工和 defect。
...
2. 增加 outcome linkage 和 reviewer policy。
3. 扩展 knowledge reviewer policy 和跨文件 conflict resolution。
