...
| 方向 | 当前证据 | 判断 |
|------|----------|------|
| SDD | Spec 模板、spec-lint、AC 编号、approval 规则 | 路线成立，需补状态和审批证据 |
...
| Gap | 影响 | 建议 |
|-----|------|------|
| Spec 无状态字段 | 不知道 draft/approved/verified | 增加 front matter |
| Approval 不可机读 | hard gate 依赖人工约定 | `approved_by`、`approved_at`、`spec_hash` |
| Plan 未被 gate 校验 | plan 与 AC 可能脱节 | 新增 `plan-lint.sh` |
| AC 只检查命名覆盖 | 无法判断测试语义 | 增加 review/eval 记录 |
...
目标：让一个真实项目可以稳定完成 Spec -> Code -> Verify -> Evidence。

建议任务：

- README 中真实 GitHub URL 替换占位。
- `pipeline.sh --json-out` 输出 eval run JSON。
- Spec 增加 front matter schema。
- 新增 `plan-lint.sh`。
- Smoke test 覆盖 install/init/pipeline/eval JSON。
