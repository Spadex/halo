...
| Evidence 已有 pipeline/gate JSON、context-run、loop state、outcome link/report、central eval sink、可配置 failure category、escalation learn draft、knowledge review event、promotion event、knowledge metadata/governance lint、process evidence、Markdown summary、history report、CI artifact 与 PR comment 模板，但仍缺少 UI | 数据已经可汇总，团队视角仍需要可视化和查询 | dashboard |
...
| Dashboard 未沉淀 | central sink 已有文件协议，但还没有可视化 UI | dashboard |
