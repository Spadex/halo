...
Halo 当前已经达到“可试点的 repo-local AI Coding harness”阶段：安装、SDD 引导、目录化 spec、`spec.md` 内嵌 Context Basis、基础 gates 和 smoke test 都已形成闭环。
...
| Superpowers worktree isolation 未产品化 | Superpowers 的发布工作流会在设计批准后进入隔离 workspace；Halo 当前依赖宿主 Agent 或用户手工分支 | 在 adapter/host 层补 `worktree preflight`，不放进 PrismSpec 主产物契约 |
...
目标：用一个真实业务项目跑通完整 PrismSpec + Halo 流程。
...
- 在真实示例中填充 `halo/context/README.md`、`external.md` 和项目知识文件。
