...
当前实现已经具备最小可信闭环：安装、初始化、PrismSpec 引导、context 检索、验证 pipeline、基础 gates 和 smoke test。尚未完成的是结构化 eval、context 治理、插件协议和多语言 drift parser。
...
| [Context 设计](context.md) | 项目上下文、知识库、中心知识联动和防腐设计 |
...
| Query-first context | 上下文按需检索，不把全量知识常驻 prompt。 |
...
- `lattice/kernel/context/loader.sh` context 检索。
...
- context/knowledge front matter schema、stale/conflict 检查。
