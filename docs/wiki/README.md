...
当前实现已经具备最小可信闭环：安装、初始化、PrismSpec 引导、目录化 spec、`spec.md` Context Basis、spec 状态推进与 transition history、plan/task contract lint、next task resolver、evidence-gated task completion、summary draft、summary-to-learn draft、基础 context 供给、验证 pipeline、基础 gates、结构化 eval run、central eval sink、static eval dashboard、eval query、loop state、outcome link/report、可配置 failure category、failure category lint、escalation learn draft、learn draft promotion/discard、knowledge review evidence、knowledge metadata lint、knowledge governance lint、review/TDD process evidence、Markdown summary/history 和 smoke test。尚未完成的是 dashboard 趋势增强、插件协议和多语言 drift parser。
