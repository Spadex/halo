...
PrismSpec 是 Halo 中独立出来的 Spec-Driven Development skill pack。对用户呈现为五个产品板块：
...
| 模式 | 触发条件 | 产物路径 | 验证入口 |
|------|----------|----------|----------|
| Standalone | 无 `halo/manifest.yaml` | `prismspec/specs/<id>/`、`.prismspec/runs/<id>/` | 检测本地 build/lint/test 命令 |
| Halo-hosted | 存在 `halo/manifest.yaml` | `halo/specs/<id>/`、`.halo/sdd/<id>/` | `bash halo/kernel/delivery/pipeline.sh --json-out` |

Halo-hosted 模式会额外使用 manifest、项目 context 地图、verification gates 和项目配置。Standalone 模式只保留 Spec Coding 主链路，适合只想使用 skill pack 的用户。
...
- `.halo/sdd/` 与 `.prismspec/runs/` 是执行证据目录，默认不作为长期知识。
...
在 Halo-hosted 模式下，Specification 需要读取 `halo/manifest.yaml`，但目的不是把 manifest 当业务上下文，而是获取项目路由：
...
在 Halo-hosted mode 中，Implementation 应先运行：
...
```bash
bash halo/kernel/context/summary-learn-draft.sh <spec-id>
```
