...
Lattice-hosted 模式会额外使用 manifest、knowledge loader、delivery gates 和项目配置。Standalone 模式只保留 Spec Coding 主链路，适合只想使用 skill pack 的用户。

## 阶段与产物

| 阶段 | 目标 | 持久产物 | 临时证据 |
|------|------|----------|----------|
| Brainstorming | 明确 intent、scope、AC、risk、mode | `spec.md` | 需求澄清记录可写入 spec |
...
Brainstorming 是第一阶段，产物是 `spec.md`。它不是随意头脑风暴，而是把需求压缩成可执行契约。
...
Brainstorming 不应该全量复制 manifest、代码或知识库到 spec。只记录影响 Scope、AC、Risk、Execution Policy 的事实。
...
TDD mode 必须先生成 red-test tasks，再生成 implementation tasks。
...
- task brief / review package helper。
...
4. 将 guide/lint 输出接入 CI artifact。
