...
Halo 的产品工作台可以面向用户呈现为四个主要板块：

```text
Clarify -> Spec -> Build -> Quality Gate
```

这四个主要板块不是另起一套流程，而是 PrismSpec + Halo 的产品化视图。底层仍由 Agent Skills-compatible skill folders、文件产物、命令 gates 和 evidence 组成。
...
| Agent Skills 做法 | Halo / PrismSpec 对应 |
|---|---|
...
| 复杂或确定性逻辑放 `scripts/` | `prismspec/bin/` 和 `halo/kernel/` |
...
- `halo/context/README.md` context map；
...
Review 判断 intent、diff、实现证据、测试质量和风险是否允许进入最终验证。Verification 运行真实命令或 Halo pipeline，证明当前仓库状态是否可交付。
