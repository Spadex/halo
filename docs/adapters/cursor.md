...
Halo works with [Cursor](https://cursor.com) in two layers:

1. A lightweight rules-file adapter that works today.
2. A future thin Cursor control plane that should discover Halo runtime capabilities instead of hard-coding workflow details.
...
2. Create `.cursor/rules` (or `.cursorrules`) and import Halo rules:

```
# Include Halo behavior rules
@file halo/kernel/orchestrator/rules.md
```

3. Cursor will now follow Halo phase rules when generating code.

4. Check the runtime capability declaration:

```bash
bash halo/kernel/capabilities.sh --json
```
...
"Read halo/context/README.md, then run bash halo/kernel/context/backends/knowledge.sh <keywords> only if curated knowledge lookup is useful"
"Run bash halo/kernel/delivery/gates/spec-lint.sh halo/specs/my-spec/spec.md"
```

## Control Plane Contract

A Cursor plugin should stay thin. It should render a stable UI shell and delegate workflow details to the repo-local Halo runtime:

| Concern | Owner |
|---------|-------|
| Status bar, sidebar, command palette, MCP registration | Cursor plugin |
| Workflow routing, gates, metrics, reports, policies | Halo runtime |
| Capability, action, metric, and report schemas | Versioned Halo protocol |
| Specs, context, eval runs, outcomes | Target project |

The stable adapter calls are:

- `capabilities()` -> `bash halo/kernel/capabilities.sh --json`
