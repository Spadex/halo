...
```mermaid
flowchart LR
  I[Intent] --> S[Specification<br/>context.md + spec.md]
  S --> P[Planning<br/>plan.md]
  P --> E[Implementation<br/>plan or tdd]
  E --> R[Review<br/>review-summary.json]
...
| 阶段 | 关键问题 | 主要产物 | 质量门禁 |
|------|----------|----------|----------|
| Specification | 做什么？依据是什么？如何验收？ | `context.md`、`spec.md` | scope 清晰、AC 可测试、risk/mode 明确 |
...
| Review | 是否符合 spec？代码质量是否可接受？ | `review-summary.json` | spec compliance 与 code quality 分离判断 |
...
| `prismspec-specification` | 把 intent 固化为可验收契约 | `context.md`、`spec.md` |
...
| `prismspec-review` | 审查实现证据和 diff | `review-summary.json` |
...
- templates、references、commands、guide/lint/doctor helper；
