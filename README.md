...
Lattice 是面向团队的 repo-local AI Coding control plane。它内置 PrismSpec：一套风险自适应的 Spec Coding 工作流，用最小必要契约把模糊意图压缩成可执行、可审查、可验证的工程约束，并在实现阶段支持 `plan` 与 `tdd` 两种 mode。

PrismSpec 负责把一次 AI Coding 任务落到 `spec.md`、`plan.md`、`review.md`、`verify.md`；Lattice 在此基础上补充项目 context、verification gates、evidence/eval、loop/learn，让个人提效持续转化为团队可复用的工程能力。
...
一句话：**PrismSpec 提供风险自适应的 Spec Coding 契约，Lattice 把这些契约接入团队级 context、验证和证据闭环。**
...
项目可在 `lattice/manifest.yaml` 设置默认模式；用户也可以对单次 spec 指定模式。发现风险时允许 `plan -> tdd` 升级；`tdd -> plan` 需要显式用户 override 并记录风险。
...
| pipeline 跳过大多数 step | 检查 `lattice/manifest.yaml` 的 project commands，以及是否设置了 active spec |
...
Lattice 当前已经具备 **repo-local AI Coding control plane 的最小可信闭环**。适合以 preview / pilot 口径对外试用，并用于非关键仓库、新需求链路和团队内部流程验证。
