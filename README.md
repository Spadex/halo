...
| 查看 SDD 下一步 | `bash prismspec/bin/guide.sh --json` |
...
| 生成 closeout summary | `bash lattice/kernel/orchestrator/sdd/summary-draft.sh <spec-id>` |
| 从 summary 生成 learn draft | `bash lattice/kernel/context/summary-learn-draft.sh <spec-id>` |
...
| 方向 | 已可用能力 |
|------|------------|
| 安装与初始化 | `install.sh`、`init.sh`、`doctor.sh`、smoke test、GitHub Actions eval artifact 模板 |
| PrismSpec | canonical skills、`guide.sh`、`lint.sh`、多模板、Plan/TDD policy、standalone 与 Lattice-hosted 两种模式 |
| Spec lifecycle | `context.md`、`spec.md`、`plan.md`、`verify.md`、`summary.md`、状态推进、transition event/history |
