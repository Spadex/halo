...
Halo is currently an early preview project. It is suitable for non-critical repositories, team pilots, and new feature workflows while the contracts and verification coverage continue to mature.
...
- output of `bash halo/kernel/doctor.sh` when available;
...
| Doctor fails after upgrade | Re-run `install.sh --upgrade --init` from the target repository and inspect `halo/state/*-backups/`. |
| Pipeline skips most steps | Confirm `halo/manifest.yaml` has project commands and `halo/specs/<spec-id>/spec.md` exists. |
